...
static struct ibv_mr *client_metadata_mr = NULL, *server_gpu_dst_mr = NULL, *server_gpu_src_mr = NULL;//, *server_src_mr = NULL, *server_dst_mr = NULL;
static struct rdma_buffer_attr client_metadata_attr;
static struct ibv_recv_wr client_recv_wr, *bad_client_recv_wr = NULL;
static struct ibv_sge client_recv_sge;
/* Source and Destination buffers, where RDMA operations source and sink */
static void *srcgpu = NULL, *dstgpu = NULL;

				 //2^19 = 524288 2^20 = 1048576 2^21 = 2097152
//#define TOTALSIZE (TOTALSIZE) /* Total memory region size (BUFFSIZE*2) */
//#define BUFFSIZE (BUFFSIZE) /* Memory region size (RDMA Write or RDMA Read) */

/* This function does RDMA Write from src to remote buffer */
int server_rdma_write(int size)
{
	int ret = -1;
	...
	rdma_write_sge.addr = (uint64_t)server_gpu_src_mr->addr;
	rdma_write_sge.length = size;
	rdma_write_sge.lkey = server_gpu_src_mr->lkey;
	...
	rdma_write_wr.wr.rdma.remote_addr = client_metadata_attr.address;
	rdma_write_wr.wr.rdma.rkey = client_metadata_attr.stag.local_stag;
	rdma_write_wr.send_flags = IBV_SEND_SIGNALED;
	debug("\nWriting to adress: %p From adress: %p\n",(void*) client_metadata_attr.address, (void*)server_gpu_src_mr->addr);
...
/* This function does RDMA Read from remote buffer to dst not called now.*/
int server_rdma_read(int size)
{
	int ret = -1;
	struct ibv_send_wr *bad_wr = NULL;
	struct ibv_wc wc[1];
	...
	rdma_read_sge.addr = (uint64_t)server_gpu_dst_mr->addr;
	rdma_read_sge.length = size;
	rdma_read_sge.lkey = server_gpu_dst_mr->lkey;
	...
	rdma_read_wr.wr.rdma.remote_addr = client_metadata_attr.address;
...
static void setup_gpu_mr()
{
	...
	//Allocate a GPU buffer to be used by client
	server_gpu_src_mr = rdma_buffer_alloc_gpu_adress(pd,
		srcgpu, 
		TOTALSIZE,
	...
	if(!server_gpu_src_mr){
		rdma_error("Failed to register the first buffer, ret = %d \n", ret);
	}
	debug("Allocated GPUsrc memory: %p \n", srcgpu);

	server_gpu_dst_mr = rdma_buffer_alloc_gpu_adress(pd,
		dstgpu, 
		sizeof(int),
		(IBV_ACCESS_REMOTE_READ |
		 IBV_ACCESS_LOCAL_WRITE | // Must be set when REMOTE_WRITE is set.
		 IBV_ACCESS_REMOTE_WRITE));
		 // Must be set when REMOTE_WRITE is set.
		 
	if(!server_gpu_dst_mr){
		rdma_error("Failed to register dst buffer, ret = %d \n", ret);
	} 
	debug("Allocated GPUdst memory: %p \n", dstgpu);
...
static int disconnect_and_cleanup()
{
	...
	free_gpubuffer_rdma(server_gpu_src_mr);
	free_gpubuffer_rdma(server_gpu_dst_mr);
	/* We free the buffers */
	/* DO CUDA FREE HERE  <----> CUDAFREE Done*/
...
int main(int argc, char **argv)
{
	...
	/* Set src and dst */
	/* cudaAlloc( CudaMallocHost ) returns pinned memory from CPU. */
	int ret2 = cudaAlloc(&srcgpu, BUFFSIZE);
	//printf("CudaALloc on p: %p or %p\n", &srcgpu, srcgpu);
	if (ret2 != 0) {
		debug("Failed to allocate srcgpu")
		return -ENOMEM;
	}
	int ret3 = cudaAlloc(&dstgpu, sizeof(int));
	//printf("CudaALloc on p: %p or %p\n", &dstgpu, dstgpu);
	if (ret3 != 0) {
		debug("Failed to allocate dstgpu")
		cudaFreeAddr(srcgpu);
		return -ENOMEM;
	}
	...
	setup_gpu_mr();
	write_rdma_buffer_addr((void *)server_gpu_src_mr->addr);
	...
	/**
	 * @brief We write i (2000) packages of 2^21 bytes j(10) times to the receiver. We then do a RDMA_READ to see if the first element of the buffer is -1.
	 * If it is -1 we know that the receiver has received and handled the RDMA_write and we can start the next transfer. We finish at package #20000.
	 * 
	 */
	for (size_t j = 0; j < 10; j++){
		...
		for (size_t i = 1; i <= 2000; i++)
		{
			...
			((int*)server_gpu_src_mr->addr)[0] = i+j*2000;
			//printf("wrote: %d \n", i+j*2000);
			ret = server_rdma_write(BUFFSIZE);
			...
			while(!done){
				ret = server_rdma_read(sizeof(int));
				if (ret) {
					rdma_error("Failed to finish RDMA write, ret = %d \n", ret);
					return ret;
				}
				if(((int*)server_gpu_dst_mr->addr)[0] == -1 ||((int*)server_gpu_dst_mr->addr)[0] == 20000 ){
	...
	printf("Data sent over: %d bytes ", BUFFSIZE*2000);
	printf("Time elapsed: %fs, Seconds: %ld, uSeconds: %ld \n", elapsed, seconds, microseconds);
	printf("Throughput: %f megabytes/s\n ", (double)((BUFFSIZE*2000/elapsed)/1e6));
	printf("J : %ld \n",j);
	//cudaCopy((void*) client_metadata_attr.address,(void*)server_gpu_src_mr->addr, 1);
	}
